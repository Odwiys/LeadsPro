...
class AgentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create agent" do
    assert_difference("Agent.count") do
      post agents_url, params: { agent: { email: @agent.email, login: @agent.login, name: @agent.name } }
    end
  ...
  test "should update agent" do
    patch agent_url(@agent), params: { agent: { email: @agent.email, login: @agent.login, name: @agent.name } }
